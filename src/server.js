...
async function loadPaymentOptions() {
  ...
  const defaults = PAYMENT_OPTION_DEFINITIONS.map((option) => ({
    ...option,
    isEnabled: true,
    updatedAt: null,
  }));
  ...
  try {
    ...
    const optionsByKey = defaults.reduce((acc, option) => {
      acc[option.key] = option;
      return acc;
    }, {});
    ...
    console.warn('[payments] failed to load admin payment options; using defaults', error);
    return defaults;
  }
}
...
async function setPaymentOptionEnabled(optionKey, isEnabled) {
  const key = normalizePaymentOptionKey(optionKey);
  const definition = getPaymentOptionDefinitionMap()[key];
  if (!definition) {
  ...
  try {
    ...
    await prisma.$executeRaw`
      INSERT INTO admin_payment_options (option_key, is_enabled, sort_order, updated_at)
      VALUES (${key}, ${Boolean(isEnabled)}, ${definition.sortOrder}, NOW())
    ...
    return {
      ok: true,
      message: `${definition.label} ${isEnabled ? 'enabled' : 'disabled'}.`,
      option: definition,
      isEnabled: Boolean(isEnabled),
    };
  } catch (error) {
    console.error('[payments] failed to update payment option', { optionKey: key, error });
    return {
      ok: false,
      message: 'Payment option could not be updated right now. Please try again.',
      option: definition,
      isEnabled: isPaymentOptionEnabled(key),
    };
  }
}
...
const JOB_ASSIST_CATEGORY_KEYWORDS = {
  ...
  Driveways: ['driveway', 'asphalt', 'concrete', 'sealcoat', 'paver', 'resurface', 'crack fill'],
  'Decks & Porches': ['deck', 'porch', 'railing', 'stairs', 'baluster', 'stain deck', 'composite decking'],
  Roofing: ['roof', 'shingle', 'flashing', 'gutter', 'leak roof', 'roof vent', 'roof repair'],
  'Tree Service': ['tree', 'stump', 'prune', 'trim tree', 'arborist', 'branch removal', 'tree removal'],
  'Bathroom Remodel': ['bathroom remodel', 'vanity', 'tile shower', 'bath fan', 'toilet replace', 'bath update'],
  'Kitchen Remodel': ['kitchen remodel', 'countertop', 'cabinet install', 'backsplash', 'island', 'kitchen update'],
  Windows: ['window', 'glass pane', 'window frame', 'window install', 'window replacement', 'drafty window'],
  Landscaping: ['landscaping', 'sod', 'retaining wall', 'hardscape', 'irrigation', 'landscape design', 'planting bed'],
...
const JOB_ASSIST_BUDGET_BASE = {
  ...
  Plumbing: [170, 420],
  Driveways: [600, 3800],
  'Decks & Porches': [550, 3600],
  Roofing: [650, 4200],
  'Tree Service': [300, 2200],
  'Bathroom Remodel': [2500, 14000],
  'Kitchen Remodel': [5000, 28000],
  Windows: [500, 12000],
  Landscaping: [450, 4200],
  'Yard Help': [95, 280],
...
function buildJobTitle(category, prompt, description, currentTitle) {
  ...
  const fallbackByCategory = {
    ...
    Driveways: 'Driveway repair or resurfacing help needed',
    'Decks & Porches': 'Deck or porch repair and upgrade help needed',
    Roofing: 'Roofing inspection or repair help needed',
    'Tree Service': 'Tree trimming or removal help needed',
    'Bathroom Remodel': 'Bathroom remodel help needed',
    'Kitchen Remodel': 'Kitchen remodel help needed',
    Windows: 'Window repair or replacement help needed',
    Landscaping: 'Landscaping and outdoor design help needed',
