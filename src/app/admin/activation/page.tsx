...
export default async function ActivationPage() {
  ...
  return <main className="min-h-screen bg-muted/30"><div className="mx-auto max-w-7xl space-y-8 px-4 py-8 sm:px-6">
    ...
    <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">{stages.map((stage) => <Card key={stage}><CardHeader><CardDescription>{stage.replaceAll('_', ' ')}</CardDescription><CardTitle>{journeys.filter((item) => item.stage === stage).length}</CardTitle></CardHeader></Card>)}</section>
    {latest && <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><Card><CardHeader><CardDescription>Profile completion</CardDescription><CardTitle>{latest.profileCompletionRate}%</CardTitle></CardHeader></Card><Card><CardHeader><CardDescription>Reached first bid</CardDescription><CardTitle>{latest.firstBidRate}%</CardTitle></CardHeader></Card><Card><CardHeader><CardDescription>Reached first win</CardDescription><CardTitle>{latest.firstWinRate}%</CardTitle></CardHeader></Card><Card><CardHeader><CardDescription>Stalled journeys</CardDescription><CardTitle>{latest.stalled}</CardTitle></CardHeader></Card></section>}
